...
COMANDO_S1TBX='"C:\\Program Files\\S1TBX\\gpt.bat" "'        #path al bat per il command line di S1TBX
COMANDO_NEST='"C:\\DFC Slave\\S1_TEST\\gpt2.bat" "'          #path al bat per il command line di NEST
GDAL_TRANSLATE='"C:\\OSGeo4W64\\bin\\gdal_translate.exe" -of GTiff ' #path al comando gdal_translate
GDAL_OGR2OGR='"C:\\OSGeo4W64\\bin\\ogr2ogr.exe" -f "ESRI Shapefile" ' 
NEST=1  #Utilizza NEST invece che S1TBX
MAIN_CARTELLA_DEBURST='C:\\DFC Slave\\S1_TEST\\DEBURST\\'
MAIN_CARTELLA_SUBSET='C:\\DFC Slave\\S1_TEST\\SUBSET\\'
MAIN_CARTELLA_COHERENCE='C:\\DFC Slave\\S1_TEST\\COHERENCE\\'
MAIN_CARTELLA_COHERENCE_TIFF='C:\\DFC Slave\\S1_TEST\\COHERENCE_TIFF\\'
MAIN_CARTELLA_OUTPUT='C:\\FTP\\DFCData\\DFC\\'
TEMPDIR='C:\\DFC Slave\\S1_TEST\\TEMP\\'
###################################################################################################
...
def run(argsDict):
	...
	###########################  "VARIABLE" INPUTS  ###########################################################################################
	INPUTSAR=str(argsDict["INPUTSAR"])
	...
	if len(Imm)>=3:	
		...
		CARTELLA_FINALE=CARTELLA_OUTPUT+testo+str(NumeroShpCH)+'_'+A[1]+'_'+B[1]+'_'+C[1]+'_'+C[2]
		if not os.path.exists(CARTELLA_FINALE):
			os.mkdir(CARTELLA_FINALE)
		OUT_FINAL_SHP=CARTELLA_FINALE+'\\'+testo+str(NumeroShpCH)+'_'+A[1]+'_'+B[1]+'_'+C[1]+'_'+C[2]+'.shp'
	...
	return { "outDir": CARTELLA_FINALE }

if __name__ == "__main__":
        run("")
		#run({"INPUTSAR":"C:\\ProcessTest\\INPUT\\S1A_IW_SLC__1SDV_20150313T165641_20150313T165708_005016_00649C_D9AA.zip"});
